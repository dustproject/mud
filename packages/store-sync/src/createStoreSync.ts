...
import { Hex, TransactionReceipt, WaitForTransactionReceiptTimeoutError, TransactionReceiptNotFoundError } from "viem";
    ...
    const hasTransaction$ = recentBlocks$.pipe(
      concatMap(async (blocks) => {
        ...
        try {
          ...
          if (error.name === TransactionReceiptNotFoundError.name) {
            return false;
          }
          throw error;
        }
